# Анализ пользовательских отзывов на примере приложения ChatGPT
...
Для поиска похожих пользовательских отзывов использовался метод TF-IDF (Term Frequency – Inverse Document Frequency).

TF-IDF позволяет преобразовывать текстовые отзывы в числовое представление с учётом важности слов внутри корпуса.

Полученные векторные представления использовались для вычисления семантического сходства между пользовательским запросом и отзывами с помощью cosine similarity.

Файл: scripts/similarity.py


## Streamlit приложение

Для демонстрации результатов был разработан интерактивный веб-интерфейс с использованием Streamlit.

Приложение позволяет пользователю вводить слова или фразы и автоматически находить семантически похожие отзывы на основе TF-IDF и cosine similarity.

Интерфейс реализует поиск похожих пользовательских отзывов в реальном времени без применения методов машинного обучения.
